...
function BusCard({ bus }: { bus: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden group hover:-translate-y-1 flex flex-col min-h-screen py-6 px-4">
      <div className="p-6 flex-1">
        <h2 className="text-xl font-bold mb-3">{bus.bus_number}</h2>
...
function LoadingSkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="bg-white rounded-2xl shadow-lg p-6 animate-pulse h-60"></div>
      ))}
    </div>
  );
}
...
export default function BusesPage() {
  ...
  useEffect(() => {
    ...
    load();
  }, [search]); // 👈 PAGE UPDATES WHEN SEARCH CHANGES
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50 py-8">
        <div className="container mx-auto px-4 text-center mb-12">
          <h1 className="text-4xl font-bold mb-4">Available Buses</h1>
          <p className="text-gray-600 text-lg">Find the best bus for your destination</p>
        </div>
        ...
        <div className="container mx-auto px-4">
          {loading ? (
            ...
            <div className="text-center py-16">
              <p className="text-xl font-semibold">No matching buses</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {buses.map((bus) => (
